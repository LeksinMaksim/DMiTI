...
std::pair<Rationals, Polynomials> FAC_P_Q(Polynomials polinom)
{
    ...
    NaturalNumbers gcfNuminator = NaturalNumbers(ABS_Z_N(elems[0]->getNodeMultiplier().getNumerator())); // Инициализируем НОД числителей
    ...
    {
        gcfNuminator = GCF_NN_N(gcfNuminator, ABS_Z_N(elems[i]->getNodeMultiplier().getNumerator())); // Обновляем НОД числителя
