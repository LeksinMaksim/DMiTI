...
Integer SUB_ZZ_Z(Integer firstNumber, Integer secondNumber)
{
    ...
    if (firstNumberSign == 0)
        return secondNumberSign == 0 ? Integer(0) : MUL_ZM_Z(secondNumber);
    ...
    size_t compareSign =
        COM_NN_D(firstNumberNaturalForm, secondNumberNaturalForm);
    bool isTwoPositive = firstNumberSign == 2 && secondNumberSign == 2;
    if (firstNumberSign == secondNumberSign)
    {
        Integer resultInteger(((compareSign == 2 && isTwoPositive) ||
                               (compareSign == 1 && !isTwoPositive))
                                  ? 1
                                  : -1);
        resultNaturalForm =
            (((compareSign == 2 && isTwoPositive) ||
              (compareSign == 1 && !isTwoPositive)))
